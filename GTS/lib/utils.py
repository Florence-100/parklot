...
def load_demo_dataset(dataset_dir, batch_size, test_batch_size=None, **kwargs):
    data = {}
    #demo_data = np.load(os.path.join(dataset_dir, 'demo_nottingham_gts' + '.npz'))
    demo_data = np.load(os.path.join(dataset_dir, 'test' + '.npz'))
    data['x_' + 'demo'] = demo_data['x']
    data['y_' + 'demo'] = demo_data['y']

    #load train data for scaler 
    train_data = np.load(os.path.join(dataset_dir, 'train' + '.npz'))
    data['x_' + 'train'] = train_data['x']
    data['y_' + 'train'] = train_data['y']
    scaler = StandardScaler(mean=data['x_train'][..., 0].mean(), std=data['x_train'][..., 0].std())

    #data format 
    for category in ['demo', 'train']:
        data['x_' + category][..., 0] = scaler.transform(data['x_' + category][..., 0])
        data['y_' + category][..., 0] = scaler.transform(data['y_' + category][..., 0])

    data['train_loader'] = DataLoader(data['x_train'], data['y_train'], batch_size, shuffle=True)
    data['demo_loader'] = DataLoader(data['x_demo'], data['y_demo'], test_batch_size, shuffle=False)
    data['scaler'] = scaler
